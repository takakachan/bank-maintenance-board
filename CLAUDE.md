...
- **取得方式**: 既定はHTML。一覧がJS描画で読めない行だけ代替手段を使う
  - `pdf_probe`(岡崎信金) 日付規則のPDFを16並列で総当たり
  - `json_source`(静岡) お知らせ一覧のJSONを直接読む
  - `sitemap_probe`(千葉) サイトマップのURLに含まれる日付で新しいものだけ開く
...
サイトの「更新履歴」ボタンの中身は `CHANGELOG.md`。変更したら新しい日付を上に足す。
**利用者が読む文章**なので、短く・専門用語や社名を出さずに書く。
- 良い例: 「停止期間の時刻が表示されない不具合を修正」
- 悪い例: 「curl_cffiでボット対策を回避、○○社サイト風にデザイン変更」
